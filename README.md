...
- **Open Knowledge Format export** — project the wiki to a Google Cloud [OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) v0.1 bundle (`okf_export.py build wiki --output-dir <dir>`) for cross-tool / cross-org exchange. Read-only projection: CE structure rides in `x_ce_*` extension keys. Mapping and design: [`docs/okf-interop.md`](skills/curiosity-engine/docs/okf-interop.md).
- **Bootstrap densify** (opt-in, large cold vaults) — `bootstrap.py` harvests Fig/Table captions and multi-pack atomic facts at volume; catalog-only link rewrites; long CURATE still owns analyses and QA.
