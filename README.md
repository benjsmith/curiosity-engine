...
- **Open Knowledge Format export** — project the wiki to a Google Cloud [OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) v0.1 bundle (`okf_export.py build wiki --output-dir <dir>`) for cross-tool / cross-org exchange. Read-only projection: CE structure rides in `x_ce_*` extension keys; provenance/identity extension proposal in `docs/okf-provenance-ext.md`.
