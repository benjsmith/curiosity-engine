...
**Entity-resolution gate (v0.8.3).** `graph.py retrieve` (and synthesis `query_router.py classify`) resolve named entities against the curated identity layer before answering. Full abstain returns no retrieval context; pure-uncurated queries are restricted to verbatim-naming material. Prefer retrieve over ungated `vault_search` for entity-scoped questions. Details: SKILL.md QUERY §.
...
- **Phase 2b++++++:** Classic rubber-band / Shift-click split targeting in wiki-view;
  Atlas canvas rubber-band / heal / tab-open chrome still Switchbay
