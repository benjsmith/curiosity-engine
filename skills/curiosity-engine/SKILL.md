...
3. Load pages via graph retrieval first: `uv run python3 <skill_path>/scripts/graph.py retrieve wiki "<question>"` — semantic seed, multi-hop graph expansion, ranked pages with provenance, and (in blend mode) vault recall in one call. The default `--route auto` already applies the benchmark-validated policy: global/sensemaking questions get curated-graph pages only (vault chunks dilute comprehensiveness there); factoid/multi-hop questions get graph pages + a `vault` recall stream. Read the returned pages (weight wiki pages over vault chunks ~2:1 when budgeting context). Fall back to raw `uv run python3 <skill_path>/scripts/vault_search.py "query"` when you need vault-only search or FTS5 operators (`AND`, `OR`, `NOT`, `"exact phrase"`, `prefix*`, `NEAR()`, column-scoped `body:term`; default limit 10; `--text` for full bodies; `--graph-expand` fuses a third RRF stream of sources cited by wiki pages that already cite the strongest matches — useful for synthesis, skip for direct lookups).
   - **Entity abstention gate (hard rule).** `retrieve` (and `classify` on synthesis routes) emits an `entity_gate` block: every entity name in the question, deterministically resolved against the curated identity layer (page titles/stems, `aliases`, `same_as`, IRIs, wikilink pipe-aliases). Honour it strictly. `action: abstain` means the named entity does not exist anywhere in this workspace — retrieve returns **no context on purpose**; answer "there is no entity named X in this workspace", offering any `look_alike` candidate *by name only* as a different entity ("did you mean Project Onyx?"), and NEVER answer with facts that belong to a similarly-named entity — retrieval proximity is not identity. Mentions with `status: uncurated` (name occurs verbatim in raw sources but has no curated page) may be answered only from material naming them verbatim. `resolved` mentions answer normally — known aliases resolve through the identity layer, so real-entity queries are unaffected.
