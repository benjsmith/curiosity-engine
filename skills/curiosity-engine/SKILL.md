...
2. `uv run python3 <skill_path>/scripts/<named_script>.py ...` — never bare `python3`, never `-c "..."`. The `uv run` prefix auto-discovers the workspace `.venv` (created by setup.sh) so imports like `kuzu` resolve. Covers every hash-guarded skill script: `sweep.py`, `graph.py`, `lint_scores.py`, `score_diff.py`, `epoch_summary.py`, `scrub_check.py`, `naming.py`, `tables.py`, `figures.py`, `code_repo.py`, `restyle.py`, `scan.py`, plus the utility scripts `vault_index.py`, `vault_search.py`, `local_ingest.py`, `query_router.py`, `identifier_cache.py`, `shape_check.py`, `derived_cache.py`.
