...
- `retrieve wiki "<query>" [--seeds N] [--limit K] [--hops H] [--route auto|graph|blend] [--vault-k N] [--no-provisional] [--no-type-priority]` — **the primary retrieval verb**: semantic (or lexical-fallback) seed → multi-hop BFS over both edge tiers (typed edges cost 1 hop, provisional 2) → pages ranked by graph distance then query-term overlap, then **type-aware demotion** (default on: prefer `sources`/`facts`/`figures`/`tables` over long `analyses` so synthesis pages do not crowd exam/caption needles — study-sim pilot). Response includes `needle`, `type_priority`, and per-page `type_rank`. `--no-type-priority` or config `retrieve.type_priority: false` restores distance-only ordering before the limit. `--route auto` (default) routes global/sensemaking queries graph-only and blends `vault_search --mode hybrid` recall into everything else. When assembling generator context from a blend result, weight wiki pages over vault chunks roughly 2:1. **Do not** vault-first two-stage (T2) assembly — pilot showed vault fill displaces curated atoms. Runs the entity-resolution gate before seeding (abstain / pure-uncurated verbatim filter / resolved seed pin); see QUERY §.
...
- **retrieve** — `type_priority` (default `true`): after graph expansion, reorder pages so primary evidence types outrank long analyses before `--limit` is applied. Set `false` only to debug legacy distance-only ranking.
