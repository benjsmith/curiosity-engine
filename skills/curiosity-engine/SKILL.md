...
4. **Pick wave mode.** Run `uv run python3 <skill_path>/scripts/planner.py pick-mode --wiki wiki` → `{"mode", "reason", "queue_depths", "signals", "ladder"}`. Log the `reason` in the wave entry. The ladder below is the contract; `pick-mode` is its executable form, and you should use the verb rather than re-deriving the decision by hand — prose ladders are silently overridable in both directions, and a real run both mis-selected by following one and then deviated from it. If you do override, say so and why in the wave log.
   ...
   **The order is bounded-queues-before-unbounded, and that is load-bearing rather than stylistic.** Modes 1–4 are bounded and self-draining: everything they touch is marked processed (`figures.py mark-extracted`, `sweep.py mark-multimodal-extracted`, `sweep.py apply-numeric-review`) and drops off its queue, so each empties in a finite number of waves and control falls through permanently. **create** is unbounded — there is always another analysis to write — and its trigger includes `uncited_count < 5`, a number that trends to zero on a well-curated wiki and *stays* there. That is the success state, not a backlog. Placing create above a bounded queue therefore starves that queue forever; placing it below one costs at most a few waves. Exactly one of:
   ...
   - **numeric-review** — if `uv run python3 <skill_path>/scripts/sweep.py pending-numeric-review wiki` returns any candidates. These are `[tab]` pages produced by a multimodal-table-extract wave that haven't been audited yet — every multimodal extraction goes through this pass before its rows are trusted by `extracted-query`. It leads the ladder because it is not new work but an outstanding correctness obligation: pages whose numbers are unverified and which `extracted-query` already refuses to serve. Clearing that debt outranks minting more of it, and it still sequences correctly *within* a run, since a multimodal-table-extract wave populates this queue and the next wave reviews it. Dispatch one opus `numeric_transcription_review` Agent per page: each reviewer reads the source PNGs + the `[tab]` page's GFM table and returns `{verdict, flagged_cells, notes}`. Orchestrator persists each verdict via `apply-numeric-review`. `wrong` verdicts auto-overwrite (with backup) and log a rewind invocation; `suspect` verdicts annotate the page and exclude it from `extracted-query` until a curator triages. See Phase 2 for the full protocol.
