...
2. `uv run python3 <skill_path>/scripts/<named_script>.py ...` — never bare `python3`, never `-c "..."`. The `uv run` prefix auto-discovers the workspace `.venv` (created by setup.sh) so imports like `kuzu` resolve. Covers every hash-guarded skill script: `sweep.py`, `graph.py`, `entity_gate.py`, `lint_scores.py`, `score_diff.py`, `epoch_summary.py`, `scrub_check.py`, `naming.py`, `procedure_maturity.py`, `tables.py`, `figures.py`, `code_repo.py`, `restyle.py`, `scan.py`, plus the utility scripts `vault_index.py`, `vault_search.py`, `local_ingest.py`, `query_router.py`, `identifier_cache.py`, `shape_check.py`, `derived_cache.py`, `okf_export.py`, `bootstrap.py`.
...
5. **Fill the wave** with up to `parallel_workers` targets of the chosen mode.
   ...
   **Create mode — per-bucket quotas** (out of `parallel_workers` slots, default 10). Evidence is the default channel for paper findings and empirically under-populated when priority-sequenced; allocate a floor, not a leftover. Order within the wave: evidence → facts → demand promotions → summary tables → analyses.
   ...
   - **Procedure deepen (when `wiki/procedures/` exists)** — before creating/deepening procedure-shaped pages, run `procedure_maturity.py` / `sweep.py procedure-candidates` and prefer highest `spend_priority` (immature / chronology-risk hubs first). Do not invent SOP steps; set `steps_attested` from the vault.
...
type: entity | concept | source | analysis | evidence | fact | summary-table | procedure | execution
created: YYYY-MM-DD
updated: YYYY-MM-DD
...
The `title` prefix tag (`[con]`, `[ent]`, `[ana]`, `[src]`, `[evi]`, `[fact]`, `[tbl]`, `[fig]`, `[proc]`, `[exec]`) comes from `naming.TYPE_PREFIX`. **Always wrap the title value in double quotes** — strict YAML readers (PyYAML, Obsidian) parse an unquoted `[con]` as the start of a flow sequence and reject the frontmatter block entirely. The skill's own hand-rolled reader tolerates unquoted titles, but external tools do not.
...
- **procedure** — named how-to pattern: `wiki/procedures/`, title `[proc]`, plain slug. Ordered steps **only** when the vault attests them; never invent SOP steps. Optional FM: `status`, `applies_to`, `steps_attested` (bool).
- **execution** — dated instance / review / handoff of a procedure: `wiki/executions/`, title `[exec]`, stem prefix `exec-`. Put dated chatter and sync notes here, not on procedure hubs.
- Link `procedure` ↔ `execution` ↔ people/entities. Hierarchy-fit already covers “don’t invent hierarchy”.
- Before deepening procedure-shaped demand, run `uv run python3 <skill_path>/scripts/procedure_maturity.py wiki --pretty` (or `sweep.py procedure-candidates wiki`) and prefer highest `spend_priority` stems.
