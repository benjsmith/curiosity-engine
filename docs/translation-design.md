...
- **Compression interaction.** A workspace with a `compression` block configured reads pages through the inlined read-time stripping. Translation requires the worker to see the *actual* page bytes (not the stripped version) to produce a faithful translation. Operation must require the `compression` block to be relaxed/removed at workflow start, same way restyle does for prose target. The warning + offer-to-flip in `### RESTYLE` is the template.
