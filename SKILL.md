...
6. **Numeric review (mandatory).** Every multimodal-table-extract wave MUST be followed by a `numeric-review` wave on the same set of `[tab]` pages — the Opus reviewer cross-checks numeric cells against the source PNGs. See the numeric-review wave protocol for details.
