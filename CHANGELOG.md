...
- **Wiring**: SKILL.md OKF Operations block + `okf_export.py` added to the `uv run` utility-script list and both Claude Code allowlist blocks in `setup.sh`; `template/schema.md` OKF-interop note; `tests/test_okf_export.py` (fixture wiki, no network).
